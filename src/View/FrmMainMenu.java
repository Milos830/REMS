...
import Ctrl.ChooseAReport;
import Ctrl.CustomerCrtl;
import Ctrl.PropertyCtrl;
import Ctrl.SaleCtrl;
import Ctrl.EmployedCtrl;
import Data.City;
import Data.Heating;
import Data.Renting;
...
public class FrmMainMenu extends JFrame implements Observer{
	...
	public FrmMainMenu(String title)  {
		super(title);
	...
	private void initialize()
	{
		...
		btnNekretnina.setIcon(new ImageIcon(FrmMainMenu.class.getResource("/resources/iconfinder_pisa_italy_architecture_tourism_landmark_tower_travel_3465590.png")));
		...
		btnPrikazi.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String upit = "Select * from nekretnina where grad = " +((City) cbGrad.getSelectedItem()).getIDgrad();
				if(chckbxIzaberiTipNekretnine.isSelected())
				{
					upit += " and tipNekretnine =" + ((TypeOfProperty) cbTipNekretnine.getSelectedItem()).getIDtip_nekretnine();
				}
				if(chckbxIzaberiTipGradnje.isSelected())
				{
					upit += " and tip_Gradnje =" + ((TypeOfConstruction) cbTipGradnje.getSelectedItem()).getIDTip_gradnje();
				}
				if(chckbxIzaberiLokaciju.isSelected())
				{
					upit += " and Lokacija =" + ((Location) cbLokacija.getSelectedItem()).getIDlokacija();
				}
				...
				{
					upit += " and Namestenost =" + ((Furniture) cbNamestenost.getSelectedItem()).getIDnamestenost();
				}
				if(chckbxIzaberiTipGrejanja.isSelected())
				{
					upit += " and Grejanje =" + ((Heating) cbGrejanje.getSelectedItem()).getIDgrejanje();
				}
...
//							t)
	...
	for (int i =0;i<cbGrad.getItemCount();i++) {
		if((((City)cbGrad.getItemAt(i)).getIDgrad())==id) 
		{
	...
	for (int i =0;i<cbTipGradnje.getItemCount();i++) {
		if((((TypeOfConstruction)cbTipGradnje.getItemAt(i)).getIDTip_gradnje())==id) 
		{
	...
	for (int i =0;i<cbTipNekretnine.getItemCount();i++) {
		if((((TypeOfProperty) cbTipNekretnine.getItemAt(i)).getIDtip_nekretnine())==id) 
		{
	...
	public void setLokacija(int id)
	{
		for(int i=0; i<cbLokacija.getItemCount(); i++) {
			if((((Location) cbLokacija.getItemAt(i)).getIDlokacija()) == id)
			{
	...
	public void setNamestenost(int id)
	{
		for(int i=0; i<cbNamestenost.getItemCount(); i++) {
			if((((Furniture) cbNamestenost.getItemAt(i)).getIDnamestenost()) == id)
			{
	...
	public void setGrejanje(int id)
	{
		for(int i=0; i<cbGrejanje.getItemCount(); i++) {
			if((((Heating) cbGrejanje.getItemAt(i)).getIDgrejanje())== id)
			{
	...
	public void setGrejanjeCB(ArrayList<Heating> gr)
	{
	...
	public void setGradCB(ArrayList<City> gradovi)
	{
		cbGrad.removeAllItems();
		for(City grad : gradovi)
		{
			cbGrad.addItem(grad);
		}
	}
		
		public void setTipNekretnineCB(ArrayList<TypeOfProperty> tip)
		{
			for (TypeOfProperty tipNekretnine : tip) 
			{
		...
		public void setTipGradnjeCB(ArrayList<TypeOfConstruction> tipG) /// da bi se pojavio tip gradnje u kombo boksu, setovanje
		{
			for (TypeOfConstruction tipGradnje : tipG) {
				cbTipGradnje.addItem(tipGradnje);
			}
		}
		
		public void setLokacijaCB(ArrayList<Location> loka)
		{
			for (Location LOK : loka) {
				cbLokacija.addItem(LOK);
				
			}
		}
		
		public void setNamestenostCB(ArrayList<Furniture> names)
		{
			for(Furniture NAM : names)
			{
		...
		public void update(Observable arg0, Object arg1) {
			...
			{
			if(((ArrayList<Object>) arg1).get(0) instanceof Property)
	{
		ArrayList<Property> lista = (ArrayList<Property>)arg1;
